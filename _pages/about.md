...
The research direction I want to take into a PhD is **safe, decentralised coordination of mobile-robot teams in dense, human-occupied environments** — small teams of robots, some of them carrying manipulators, sharing a workspace with humans and with each other under no physical barriers, with coordination and safety properties that hold up under realistic sensor, network, and mechanical fault. I have built the layers this question touches: a production 3-robot AGV fleet running decentralised prioritised path planning for months in a live garment factory ([deployment]({{ '/projects/01_agv_fleet/' | relative_url }})); the embedded drive and comms substrate that coordination silently depends on — a first-author manuscript under review on GA-tuned bilateral cross-coupled BLDC synchronisation ([paper]({{ '/projects/04_ga_pid/' | relative_url }})); and a 6-DOF mobile manipulator with a from-scratch closed-form IK on Jontro Soinik 2.0, Cybernetics' lightweight, man-transportable next-generation EOD ROV ([manipulator]({{ '/projects/02_manipulator/' | relative_url }})). What I want to add in a PhD is the methodological rigor that field-deployed engineering rarely produces on its own: turning a coordination layer that empirically works into one with a *characterised* safety guarantee — scaled past demonstrative fleet sizes, and characterised down to the embedded substrate it relies on. The thesis direction, a concrete first-year plan, and how I would close the formal-methods gap are on the [research statement]({{ '/research-statement/' | relative_url }}) page.
