...
img: assets/img/projects/agv-fleet-in-operation.jpg
importance: 2
category: professional
related_publications: true
---

{% include figure.liquid path=page.img class="project-hero-img rounded z-depth-1" alt="Two AGVs of the fleet running a carton-transport cycle on the live factory floor among workers." %}

<p class="text-center"><em>The fleet in production — a loaded robot running a delivery leg while an empty peer returns, sharing the aisle with workers and with each other, no barriers and no floor marshal. An AprilTag localization column stands at the right edge of the aisle.</em></p>
...
**Where this sits in the multi-agent planning literature.**
This is a **decentralised variant of prioritised multi-agent path-finding (MAPF)** with a peer-published reservation table — closest in spirit to cooperative A\* (Silver, 2005) and prioritised planning, and a deliberately different design point from optimal centralised solvers like CBS / ECBS or coupled lifelong methods like PIBT. The three peer-conflict rules above map directly onto the standard MAPF conflict taxonomy: **vertex conflict** (single-cell occupancy at time *t*), **swap conflict** (two robots exchanging cells across a single edge), and **target conflict** (one plan terminating inside another robot's goal cell). Trade-offs are the usual ones for prioritised schemes: incomplete in the general case (priority cycles can deadlock — handled here by the FleetCore deadlock-recovery layer described above), and per-robot priorities are inherited from job-dispatch order rather than searched. In exchange we get a fully decentralised planner that scales linearly in robots and recovers gracefully from intermittent network drops — the right point on the curve for a 3-robot factory deployment, and the right baseline to grow from before reaching for centralised optimal solvers.
